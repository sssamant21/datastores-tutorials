...
SELECT pg_is_in_recovery() AS is_in_recovery,
       ...
       CASE WHEN pg_is_in_recovery() THEN pg_last_xact_replay_timestamp() END AS last_replayed_xact_timestamp,
       current_timestamp AS observed_at;
...
SELECT wal_records, wal_fpi, wal_bytes, pg_size_pretty(wal_bytes) AS wal_bytes_pretty,
       stats_reset, current_timestamp AS observed_at
FROM pg_stat_wal;
...
SELECT status, receive_start_lsn, written_lsn, flushed_lsn, received_tli,
       ...
       slot_name,
       CASE WHEN sender_host IS NULL THEN '<not exposed>' ELSE '<present>' END AS sender_host_evidence,
       sender_port
...
\echo 'max_slot_wal_keep_size = -1 means UNBOUNDED SLOT WAL RETENTION.'
\echo 'No slot is created, advanced, or dropped by this artifact.'

\echo ''
\echo '[10/12] Recovery dependency inventory'
SELECT 'tablespace' AS dependency_type, spcname AS dependency_name,
       CASE WHEN pg_tablespace_location(oid) = '' THEN '<built-in/default>'
            ELSE '<external/non-default>' END AS detail
FROM pg_tablespace
UNION ALL
...
dbs AS (SELECT count(*) FILTER (WHERE datallowconn AND NOT datistemplate) AS connectable_database_count FROM pg_database),
repl AS (SELECT count(*) AS sender_count,
...
recv AS (SELECT count(*) AS receiver_count,
                count(*) FILTER (WHERE status='streaming') AS streaming_receiver_count
         FROM pg_stat_wal_receiver),
slots AS (SELECT count(*) AS slot_count,
...
SELECT role_state.is_in_recovery, dbs.connectable_database_count,
       repl.sender_count, repl.streaming_sender_count, repl.synchronous_sender_count,
       recv.receiver_count, recv.streaming_receiver_count,
...
FROM role_state CROSS JOIN dbs CROSS JOIN repl CROSS JOIN recv CROSS JOIN slots CROSS JOIN arch;

\echo ''
\echo '[12/12] Acceptance boundary'
\echo 'PostgreSQL-visible evidence collection is complete.'
\echo 'This artifact DOES NOT prove external backup readability, manifest validity, required WAL/timeline completeness, PITR/restore/application success, RPO/RTO achievement, DR infrastructure usability, IAM/KMS/secrets availability, or failover/failback rehearsal.'
