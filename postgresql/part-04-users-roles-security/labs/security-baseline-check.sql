...
SELECT d.datname, pg_get_userbyid(d.datdba) AS owner_name,
       d.datallowconn, d.datconnlimit, d.datacl
FROM pg_database AS d ORDER BY d.datname;

\echo '5. Non-system schema ownership and ACL baseline'
SELECT n.nspname AS schema_name, pg_get_userbyid(n.nspowner) AS owner_name, n.nspacl
...
SELECT n.nspname AS schema_name, c.relname AS relation_name,
       a.attname AS column_name, a.attacl
...
SELECT n.nspname AS schema_name, p.proname AS routine_name,
       ...
       p.prosecdef AS security_definer, p.proacl, p.proconfig
...
SELECT pg_get_userbyid(d.defaclrole) AS creator_role,
       ...
       d.defaclobjtype,
       CASE d.defaclobjtype WHEN 'r' THEN 'relation' WHEN 'S' THEN 'sequence'
       WHEN 'f' THEN 'function' WHEN 'T' THEN 'type' WHEN 'n' THEN 'schema'
       WHEN 'L' THEN 'large object' ELSE d.defaclobjtype::text END AS object_type,
       d.defaclacl
...
SELECT n.nspname AS schema_name, p.proname AS routine_name,
       ...
       WHEN 'a' THEN 'aggregate' WHEN 'w' THEN 'window function'
       ELSE p.prokind::text END AS routine_kind,
       pg_get_userbyid(p.proowner) AS owner_name, p.proacl, p.proconfig
FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
...
FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
WHERE p.prokind IN ('f','p') AND n.nspname NOT LIKE 'pg_%'
  AND n.nspname <> 'information_schema'
ORDER BY n.nspname, p.proname, pg_get_function_identity_arguments(p.oid);

\echo '18. Interpretation reminder'
SELECT 'This output is application-focused database/catalog baseline evidence, '
