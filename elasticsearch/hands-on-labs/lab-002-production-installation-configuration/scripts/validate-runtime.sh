...
jq -e '(.nodes | length)==3 and all(.nodes[]; (.process.max_file_descriptors | type)=="number" and .process.max_file_descriptors>=65535)' <<<"$fd" >/dev/null || { echo 'FAIL: file descriptors'; exit 1; }
